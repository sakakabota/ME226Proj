...
void setup() {
  ...
  while (getMovingAverage(positionPotPin, posHistory) > POS_MIN) {
    int currentHomingPos = getMovingAverage(positionPotPin, posHistory);
    Serial.print("Homing... Current Pos: ");
    Serial.println(currentHomingPos);
    
    retract(200);
    delay(50); // Increased delay to help Serial stability
  }
...
void loop() {
  ...
  // Constant Position Print for debugging
...
void handleButton() {
  ...
  if (buttonDown && currentState != RETRACTING) currentState = DISPENSING;
  if (!buttonDown && lastBtnState) {
